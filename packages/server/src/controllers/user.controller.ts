...
const ALLOW_REGISTRATION = process.env.ALLOW_REGISTRATION as string;
const ALLOW_REGISTRATION_FLAG = ALLOW_REGISTRATION === "true";
...
export const register = async (req: Request, res: Response) => {
  if (!ALLOW_REGISTRATION_FLAG) return;
  ...
  try {
    ...
    if (existingUser) {
      return res.status(409).json("Existing User. Please login");
    }
